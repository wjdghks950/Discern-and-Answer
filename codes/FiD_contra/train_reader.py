...
def train(model, optimizer, scheduler, step, train_dataset, eval_dataset, opt, collator, best_dev_em, checkpoint_path):
    ...
    while step < opt.total_steps:
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Training FiD Reader", total=len(train_dataloader)):
            step += 1
            (idx, labels, _, context_ids, context_mask, labels_cor) = batch
            ...
            if step % opt.eval_freq == 0:
                dev_em, accscore = evaluate(model, eval_dataset, tokenizer, collator, opt)
                model.train()
                if opt.is_main:
...
def evaluate(model, dataset, tokenizer, collator, opt):
    ...
    accscore = (sum(true_pos_list) + sum(true_neg_list)) / (sum(true_pos_list) + sum(true_neg_list) + sum(false_pos_list) + sum(false_neg_list))
    return exactmatch, accscore
...
if __name__ == "__main__":
    ...
    train_dataset = src.data.Dataset(train_examples, opt.n_context, config=opt)
    # use golbal rank and world size to split the eval set on multiple gpus
    ...
    eval_dataset = src.data.Dataset(eval_examples, opt.n_context, config=opt)
