...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value_state) in enumerate(zip(self.block, past_key_value_states)):
            ...
            if self.is_decoder and output_attentions:
                # print("layer_outputs: ", len(layer_outputs), " / cross-attention weights: ", layer_outputs[-2].shape, " / output_attentions: ", output_attentions)
                # TODO: Cross attention is calculated only in the first layer
                if layer_outputs[-2].shape[-1] == 1000:
                    cross_attention_weights = layer_outputs[-2]
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    def __init__(self, config):
        ...
        decoder_config = copy.deepcopy(config)
        # # TODO: decoder_config.output_attentions = True - experimental purposes
    ...
    Examples::
        ...
        hidden_states = encoder_outputs[0]
        #pert_weights = self.cor_classify(hidden_states, labels_cor=None, is_training=True)
        #pert_weights = softmax(pert_weights,dim=2)[:,:,0]

        #hidden_states= (hidden_states.view(1,pert_weights.shape[1],-1) * pert_weights.T).view(1,-1,hidden_states.shape[-1])
...
#         print("hidden_states", hidden_states[:,:self.encoder.n_passages*self.encoder.p_length].shape)
#         print("attention_mask", attention_mask[:,:self.encoder.n_passages*self.encoder.p_length].shape)
        ...
        decoder_outputs = self.decoder(
            ...
            inputs_embeds=decoder_inputs_embeds,
            past_key_value_states=decoder_past_key_value_states,
            encoder_hidden_states=hidden_states[:,:self.encoder.n_passages*self.encoder.p_length],
            encoder_attention_mask=attention_mask[:,:self.encoder.n_passages*self.encoder.p_length],
            head_mask=head_mask,
            use_cache=use_cache,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
